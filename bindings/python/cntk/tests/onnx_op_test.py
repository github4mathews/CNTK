...
def verify_sequence_model(model, data, tmpdir, name, device=None, loaded_model=None, resave = True, bypass_load_from_onnx = False):
    # data here is reference to the outside data object. create deepcopy to avoid changing the outside data since it might get reused.
    data = deepcopy(data)

    # onnx does not specify sparse tensor. to run imported model, a sparse matrix needs to be converted to a dense matrix 
    if bypass_load_from_onnx:
        dataOnnx = data
        loaded_model, onnx_model, test_model_path, test_data_path = create_and_populate_onnx_test_case_with_model_conversion(model, tmpdir, name, model, resave)
    else:
        dataOnnx = None
...
def test_SequenceIsFirst(tmpdir):
    ...
    verify_sequence_model(model, data, tmpdir, 'SequenceIsFirst', bypass_load_from_onnx=True)
...
def test_SequenceIsLast(tmpdir):
    ...
    verify_sequence_model(model, data, tmpdir, 'SequenceIsLast', bypass_load_from_onnx=True)
